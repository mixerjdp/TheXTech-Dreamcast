...
{
#ifdef __DREAMCAST__
    dc_install_crash_handler();

    // The MDX PGE-X parser throws callback_interrupt as ordinary control flow to
    // stop after a file's header — which is exactly what the episode menu does
    // when listing worlds, and what level loading does too. C++ exceptions do
    // not unwind on this toolchain (see eh_frame_dreamcast.cpp), so that throw
    // would kill the machine. The legacy parser reads the same files without
    // throwing on the success path.
    FileFormats::g_use_legacy_pgex_parser = true;

    // Startup progress marker; see boot_probe_dreamcast.cpp.
    dc_boot_probe(DC_PROBE_GREEN);
#endif
...
//                }
    ...
    AppPathManager::initAppPath();
    dc_boot_probe(DC_PROBE_YELLOW);

    OpenConfig();
    dc_boot_probe(DC_PROBE_WHITE);
    ...
    Controls::Init();
    Controls::LoadConfig();
    dc_boot_probe(DC_PROBE_MAGENTA);
