...
#ifdef THEXTECH_DC_STOP_AT
// Bisección de FindWorlds: un único binario, punto de parada elegido con
// -DTHEXTECH_DC_STOP_AT=N. Verde = se alcanzó ese punto; negro = murió antes.
#   include "core/dreamcast/boot_probe_dreamcast.h"
#   define DC_STEP(n) do { if((n) == THEXTECH_DC_STOP_AT) dc_boot_halt(0x07E0u); } while(0)
#else
#   define DC_STEP(n) do {} while(0)
#endif
...
static void s_LoadSingleWorld(const std::string& epDir, const std::string& fName, WorldData& head, TranslateEpisode& tr, bool editable)
{
    ...
    bool dc_parsed = FileFormats::OpenWorldFileHeaderT(in, head);
    DC_STEP(13);
