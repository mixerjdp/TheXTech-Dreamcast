...
{
#ifdef __DREAMCAST__
    // Everything below is SDL/MixerX specific and compiled out of no-SDL
    // builds, but the Dreamcast does have a real mixer backend on the AICA
    // (lib/sdl_proxy/dreamcast/mixer_dreamcast.cpp). Bring up just what the
    // rest of the sound system needs, or g_mixerLoaded stays false and
    // InitSound() returns without ever loading an effect.
    //
    // No SFX thread: PGE_NO_THREADING maps PlaySfx onto PlaySfx_Blocking.
    if(g_mixerLoaded)
        return;
    ...
    {
        pLogCritical("Dreamcast: could not open the AICA mixer, continuing without audio");
        return;
    }
