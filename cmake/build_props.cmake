...
elseif(NOT MSVC)
    if(EMSCRIPTEN)
        ...
        elseif(DREAMCAST)
            # No --gc-sections here. KallistiOS's linker script does not KEEP the
            # constructor lists or its weak init-flag table, so section GC drops
            # the C++ static initialisers and the kernel's startup registrations,
            # and the binary faults the moment the BIOS jumps to it.
            string(REGEX REPLACE "-ffunction-sections" ""
                CMAKE_C_FLAGS_RELEASE "${CMAKE_C_FLAGS_RELEASE}")
            string(REGEX REPLACE "-ffunction-sections" ""
                CMAKE_CXX_FLAGS_RELEASE "${CMAKE_CXX_FLAGS_RELEASE}")
            string(REGEX REPLACE "-fdata-sections" ""
                CMAKE_C_FLAGS_RELEASE "${CMAKE_C_FLAGS_RELEASE}")
            string(REGEX REPLACE "-fdata-sections" ""
                CMAKE_CXX_FLAGS_RELEASE "${CMAKE_CXX_FLAGS_RELEASE}")
        elseif(NINTENDO_DS)
